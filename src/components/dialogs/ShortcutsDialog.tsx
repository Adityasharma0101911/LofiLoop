  ...
  return (
    <Dialog open={open} onClose={ui.closeDialog} title="Shortcuts and tips" size="lg">
      <div className="grid gap-6 sm:grid-cols-2">
        {groups.map((group) => (
          <section key={group}>
            <h3 className="mb-2 text-[10px] font-semibold tracking-widest text-fg-subtle uppercase">{group}</h3>
              ...
              {SHORTCUTS.filter((s) => s.group === group).map((s) => (
                <div key={s.action} className="flex items-center justify-between gap-4 text-sm">
                  ...
                  <dd className="flex gap-1">
                    {s.keys.map((k) => (
                      <kbd key={k} className="min-w-6 rounded-md border border-line-strong bg-surface-2 px-1.5 py-0.5 text-center font-mono text-[11px]">
                        {k === 'Mod' ? (mac ? '⌘' : 'Ctrl') : k}
                      </kbd>
                    ))}
                  </dd>
        ...
        <section className="sm:col-span-2">
          <h3 className="mb-2 text-[10px] font-semibold tracking-widest text-fg-subtle uppercase">Mouse and touch</h3>
          <dl className="grid gap-1.5 sm:grid-cols-2">
            {TIPS.map(([how, what]) => (
              <div key={how} className="rounded-lg bg-surface-2 px-3 py-2 text-sm">
